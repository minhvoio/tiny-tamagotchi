...
| **__SEED__ harness** | The test-only `__SEED__` harness from Phase 4 stays unchanged. It does not write `name` or `lastTickAt`. The naming form still gates the app on seed runs because `state.name === ''`. |
