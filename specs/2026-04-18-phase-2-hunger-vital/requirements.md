...
| **Actions**            | Co-located in `src/game/state.ts` (no separate `actions.ts`): exports `Action` union limited to `{ type: 'FEED' } \| { type: 'TICK'; elapsedMs: number }`. Other action names (`PLAY`, `REST`, `HEAL`, `RESET`) are **not** introduced in Phase 2.                                                      |
...
| **Clamp helper**       | A small `clamp(value, min, max)` helper lives in `src/game/util.ts` (dedicated file; not in `reducer.ts`) so later phases can reuse it without importing the reducer module.                                                                                                                            |
