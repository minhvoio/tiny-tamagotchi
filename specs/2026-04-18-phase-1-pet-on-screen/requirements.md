...
| **Tests**                  | React Testing Library test (`tests/components/Pet.test.tsx`) asserts the pet renders with the expected `aria-label`. A second test (`tests/components/PetStage.test.tsx`) asserts the stage renders its children.      |
