...
| **Auto-wake**          | On the same TICK that brings energy to 100 while resting, the reducer sets `isResting=false` **in the same returned state object** — a single reducer call must clamp energy and clear the flag atomically. A dedicated reducer test (named in the Tests row) asserts both fields on one output, not across two dispatches.                                                               |
